"""Demo script for DDIM sampling with visualization.

DDIM generates sharper images faster than DDPM by using fewer sampling steps.
"""
import torch
import os
...
def sample_and_visualize(
    ...
    channels: int = 3,
    base_ch: int = 64,
    output_dir: str = "samples_ddim",
    device: Optional[torch.device] = None
):
    """Generate samples using DDIM and save visualizations.
    
    Args:
        model_path: path to model checkpoint (None for random initialization)
        num_samples: number of samples to generate
        timesteps: number of diffusion timesteps (for beta schedule)
        ddim_steps: number of DDIM sampling steps (much less than timesteps)
    ...
    print(f"DDIM sampling with {ddim_steps} steps (instead of {timesteps})")
    ...
    print(f"Sampling took {elapsed:.2f} seconds ({elapsed/num_samples:.2f}s per image)")
    
    # Save grid
    grid_path = os.path.join(output_dir, f"ddim_grid_steps{ddim_steps}_eta{eta}.png")
    ...
    for idx in range(min(num_samples, 16)):  # save first 16 individual samples
        ...
        img_path = os.path.join(output_dir, f"ddim_sample_{idx:03d}.png")
        img.save(img_path)
...
def main():
    parser = argparse.ArgumentParser(description="DDIM Sampling Demo")
    ...
    parser.add_argument("--output-dir", type=str, default="samples_ddim", help="Output directory")
